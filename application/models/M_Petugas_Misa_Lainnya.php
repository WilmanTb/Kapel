...
class M_Petugas_Misa_Lainnya extends CI_Model
{
    function get_jadwal_misa($id_admin)
    {
        ...
        WHERE p.id_petugas = '$id_fakultas' AND p.is_fakultas = 0");
    ...
    function get_petugas_misa($id_admin)
    {
        ...
        $query = $this->db->query("SELECT L.name as nama_lainnya FROM tbl_lainnya L WHERE L.id = '$id_fakultas'");
        ...
        $result = $this->db->query("SELECT p.* ,
            ...
            CASE WHEN p.is_misa = 1 
                THEN (SELECT MHS.nama FROM tbl_misa M LEFT JOIN tbl_role_petugas RP On RP.id_ibadah = M.id LEFT JOIN tbl_keanggotaan MHS ON RP.bacaan = MHS.id WHERE M.id = p.id_kegiatan  AND M.tanggal_ibadah > CURRENT_DATE())
                ELSE ''
            END as Bacaan,

            CASE WHEN p.is_misa = 1 
                THEN (SELECT MHS.nama FROM tbl_misa M LEFT JOIN tbl_role_petugas RP On RP.id_ibadah = M.id LEFT JOIN tbl_keanggotaan MHS ON RP.mazmur = MHS.id WHERE M.id = p.id_kegiatan  AND M.tanggal_ibadah > CURRENT_DATE())
                ELSE ''
            END as Mazmur,

            CASE WHEN p.is_misa = 1 
                THEN (SELECT MHS.nama FROM tbl_misa M LEFT JOIN tbl_role_petugas RP On RP.id_ibadah = M.id LEFT JOIN tbl_keanggotaan MHS ON RP.doa_umat = MHS.id WHERE M.id = p.id_kegiatan  AND M.tanggal_ibadah > CURRENT_DATE())
                ELSE ''
            END as Doa,
        ...
        END as Persembahan,
            ...
            CASE WHEN p.is_misa = 1 
                THEN (SELECT MHS.nama FROM tbl_misa M LEFT JOIN tbl_role_petugas RP On RP.id_ibadah = M.id LEFT JOIN tbl_keanggotaan MHS ON RP.dirigen = MHS.id WHERE M.id = p.id_kegiatan  AND M.tanggal_ibadah > CURRENT_DATE())
                ELSE ''
            END as Dirigen,

            CASE WHEN p.is_misa = 1 
                THEN (SELECT MHS.nama FROM tbl_misa M LEFT JOIN tbl_role_petugas RP On RP.id_ibadah = M.id LEFT JOIN tbl_keanggotaan MHS ON RP.organis = MHS.id WHERE M.id = p.id_kegiatan  AND M.tanggal_ibadah > CURRENT_DATE())
                ELSE ''
            END as Organis,
    ...
    function get_all_mahasiswa($id)
    {
        ...
        if(strpos(strtolower($nama_lainnya), "asrama") !== false)
        {
            ...
            $query = $this->db->query("SELECT * FROM tbl_keanggotaan f WHERE f.id_lainnya = '$id_fakultas'");
            return $query;
    ...
    {
        $query = $this->db->query("SELECT f.id as id_lainnya FROM tbl_lainnya f WHERE f.admin = '$id'");
        $id_lainnya = $query->row()->id_lainnya;
